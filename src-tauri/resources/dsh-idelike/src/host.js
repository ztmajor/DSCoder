// dsh-idelike（文件树 + 标签页 + 终端插件）— host half（静态 bundle 形态）
// 业务：① 终端 PTY 生命周期（经 ctx.subprocess.spawnTerminal，node-pty 真实终端）；
//       ② 本地文本文件读写与目录浏览（供 client 的"右侧文件树 / 可编辑文件标签页"使用）。
// RPC：webServer HTTP 路由（POST /_dsh/dsh-idelike/<method>，JSON 进出，同源防护）。
...
// ---------- 常量 ----------
const ROUTE_PREFIX = '/_dsh/dsh-idelike'
