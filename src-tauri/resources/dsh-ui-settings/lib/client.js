...
function resolveValue(value) {
  ...
  return {
    ...
    usageQuotaEnabled: value?.usageQuotaEnabled ?? true,
...
function runtimeStyleElement() {
  const id = 'dsh-ui-settings-runtime';
  let style = document.querySelector(`style[data-plugin-css="${id}"]`);
  if (style === null) {
    style = document.createElement('style');
    style.dataset.plugin = 'dsh-ui-settings';
...
function SettingsSection({ controller }) {
  ...
  // ===== 界面透明度功能：暂时停用（源码保留）=====
  // 原：面板挂载/透明度变化时应用设置界面透明度（宿主重建面板 DOM 后也会重新应用）
  ...
  const togglePriceBar = (next) => {
    void controller.set('usageQuotaEnabled', next).catch(() => {});
    window.dispatchEvent(new CustomEvent('dsh-usage-quota:visibility', { detail: { enabled: next } }));
  };
  ...
  const restoreFeaturesSection = () => {
    ...
    void controller.unset('usageQuotaEnabled').catch(() => {});
    window.dispatchEvent(new CustomEvent('dsh-usage-quota:visibility', { detail: { enabled: true } }));
  };
  ...
  return React.createElement('div', { className: 'dut-settings' },
    ...
    React.createElement('section', { className: 'dut-panel dut-grid' },
      ...
      React.createElement(Field, { label: I18N.priceBar, hint: I18N.priceBarHint },
        React.createElement(Seg, {
          value: resolved.usageQuotaEnabled, disabled: !writable,
          options: [
...
const TIMELINE_CSS = `
.dutl-nav{-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;z-index:25;display:flex;position:fixed;align-items:center;justify-content:flex-end;pointer-events:auto}
...
/* 非聊天视图隐藏时间线：时间线挂 body、idelike 挂宿主 shell.overlay，二者不在同一层叠上下文，
   z-index 无法比较；改为按“当前激活 tab 是否聊天”判断——idelike 激活了文件 tab
   （.dtt-tab.dtt-tab-file.active）即隐藏导航轨与悬停气泡（依赖 dsh-idelike 的类名，Chromium :has()） */
body:has(.dtt-tab.dtt-tab-file.active) .dutl-nav,
body:has(.dtt-tab.dtt-tab-file.active) .dutl-bubble{display:none !important}
`;

function installTimelineStyles() {
  const id = 'dsh-ui-settings-timeline';
  const existing = document.querySelector(`style[data-plugin-css="${id}"]`);
  if (existing !== null) return () => {};
  const style = document.createElement('style');
  style.dataset.plugin = 'dsh-ui-settings';
...
module.exports = {
  ...
  async apply(ctx) {
    ...
    if (slots === undefined) {
      console.warn('[dsh-ui-settings] slots 服务 18s 内未就绪，界面调整未注册');
      return;
    }

    ctx.effect(installBaseStyles, 'dsh-ui-settings: base styles');
    ctx.effect(installTimelineStyles, 'dsh-ui-settings: timeline styles');
    ...
    ctx.effect(() => {
      ...
      return dispose;
    }, 'dsh-ui-settings: runtime css');

    // 文本调节快捷键（默认 Ctrl + = / Ctrl + -，可在设置「文本与表格」中改绑）
    ctx.effect(() => installHotkeys(controller), 'dsh-ui-settings: hotkeys');

    // ===== 界面透明度功能：暂时停用（源码保留）=====
    // 原：事件驱动的全局信号——设置面板打开必然伴随“DOM 插入 / 属性变化 / 焦点移入 / 点击”之一，
    // 任一信号触发即探测并应用透明度（rAF 节流 + 冷却延后，不丢弃面板打开信号），无需轮询、零空转。
    // 解决“重启后第一次点开设置（默认 tab 不是「界面调整」）不生效”的问题。
    // 重新启用时：取消本段注释即可（依赖的 findSettingsLayer / findOutermostLayer /
    // applySettingsOpacity / readSettingsLayerCache / writeSettingsLayerCache 等函数均保留在下方源码中）
    // ctx.effect(() => {
    ...
    //     if (settleTimer !== 0) clearTimeout(settleTimer);
    //   };
    // }, 'dsh-ui-settings: settings layer watch');
    // ===== 界面透明度功能：暂时停用（结束）=====
