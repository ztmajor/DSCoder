// Usage Quota（用量配额插件）— host half（静态 bundle 形态）
// 业务：余额真实 API / 峰谷定价 / llm/stream 记账 / 会话聚合 / 显示名识别 / 订阅额度显示
// RPC：webServer HTTP 路由（GET/POST /_dsh/dsh-usage-quota/<method>，JSON 进出，同源防护）
// 依赖：inject ['credentials', 'timer']；可选服务 webServer（ctx.inject 等待）
// 记账持久化：usageRecords 落盘 ~/.dsh/dsh-usage-quota/usage-records.json（可用环境变量
// DSH_USAGE_QUOTA_DATA_DIR 覆盖目录），重启后真实累计花费不丢失。
...
const DATA_DIR = process.env.DSH_USAGE_QUOTA_DATA_DIR || join(homedir(), '.dsh', 'dsh-usage-quota')
const DATA_FILE = join(DATA_DIR, 'usage-records.json')
const PACKAGE_FILE = new URL('../package.json', import.meta.url)
const UPDATE_REGISTRY_URL = 'https://registry.npmjs.org/dsh-usage-quota/latest'
...
const CODEX_AUTH_FILE = process.env.DSH_USAGE_QUOTA_CODEX_AUTH || join(homedir(), '.codex', 'auth.json')
const OPENCODE_AUTH_FILE = process.env.DSH_USAGE_QUOTA_OPENCODE_AUTH || join(homedir(), '.local', 'share', 'opencode', 'auth.json')
...
export default {
  ...
  apply(ctx) {
    ...
    function flushSave() {
      ...
      } catch (err) { /* 落盘失败不影响主流程；保留 dirty，下次记账/卸载时重试 */ console.warn('[dsh-usage-quota] 记账落盘失败', String((err && err.message) || err)); }
    }
    ...
    ctx.on('llm/stream', async function* (options, next) {
      ...
      try {
        ...
        console.warn('[dsh-usage-quota] llm/stream 获取失败，本次不记账', String((err && err.message) || err));
    ...
    const ROUTE_PREFIX = '/_dsh/dsh-usage-quota';
    const ROUTES = {
